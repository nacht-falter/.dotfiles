...
if ! spt s "$artist" -a | grep "$artist"; then
	notify-send "Spotify" "Artist $artist not found" -t "$notification_timeout" -i ~/snap/spotify/spotify-icon.png
	exit
fi
...
if [ -z "$album" ]; then
	...
	notify-send "Spotify" "Playing top tracks by $artist" -t "$notification_timeout" -i ~/snap/spotify/spotify-icon.png
...
# Otherwise, search album and filter by artist, then play it
	...
	if [ -z "$response_albums" ]; then
		notify-send "Spotify" "Album $album not found" -t "$notification_timeout" -i ~/snap/spotify/spotify-icon.png
		exit
	fi
	filtered_albums=$(echo "$response_albums" | grep -i ~"$artist:")
	echo "$filtered_albums"
	if [ -z "$filtered_albums" ]; then
		notify-send "Spotify" "Album $album by $artist not found" -t "$notification_timeout" -i ~/snap/spotify/spotify-icon.png
		exit
	fi
